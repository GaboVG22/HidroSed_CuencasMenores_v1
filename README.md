# HidroSed Cuencas Menores v1.0 · < 100.000 km²

Aplicación especializada para **cuencas menores a 100.000 km²**.

## Criterio técnico

Esta versión mantiene la calidad técnica de la aplicación maestra y permite trabajar con mayor detalle topográfico cuando el tamaño del DEM lo permita.

## Uso recomendado

- Cuencas pequeñas/medias: curvas 5 m, 10 m, 25 m.
- Cuencas grandes dentro de este rango: curvas 25 m, 50 m, 100 m.
- Curvas de 1 m: usar solo después de validar DEM y cuenca.
...
## Verificación preventiva v1.1

Se revisaron y corrigieron los errores ya detectados en el proyecto:

- `recommended_tiling` importado correctamente.
- Curvas de apoyo topográfico siguen siendo opcionales.
- Curvas por teselas usan `cs.allsegs`, no `cs.collections`.
- Distancia entre curvas visible y seleccionable desde 1 m.
- Se elimina el término confuso `Límite QA`.
- Se elimina `packages.txt` para evitar builds lentos por apt/GDAL.
- Se elimina `scikit-image` como dependencia obligatoria.
